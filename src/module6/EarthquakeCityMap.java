...
public class EarthquakeCityMap extends PApplet {
	...
	private void sortAndPrint(int numToPrint) {
		 ...
		 * (hint: there is a method in the List interface named toArray() which returns
		 * the elements in the List as an array of Objects).
		 * 
		 * Then it will sort the array of earthquake markers in reverse order of their
		 * magnitude (highest to lowest) and then print out the top numToPrint
		 * earthquakes. If numToPrint is larger than the number of markers in
		 * quakeMarkers, it should print out all of the earthquakes and stop, but it
		 * should not crash.Call this method from setUp() to test it. An example input
		 * and output files are provided in the data folder: use test2.atom as the input
		 * file, and sortandPrint.test2.out.txt is the expected output for a couple
		 * different calls to sortAndPrint.
		 * 
		 * 
		 * 
		 */
		// int sizeOfQuakesArray = quakeMarkers.size();
		// if (numToPrint > sizeOfQuakesArray) {
		// numToPrint = sizeOfQuakesArray;
		// }
		//
		// if ( quakeMarkers.get(0) instanceof EarthquakeMarker) {
		//
		// quakeMarkers = quakeMarkers.stream().sorted().collect(Collectors.toList());
		//
		// Object [] earthQuakesArray = quakeMarkers.toArray();
		// for ( int i = 0; i < numToPrint; i++) {
		// System.out.println(earthQuakesArray[i]);
		// }
		// }
	...
	private List<EarthquakeMarker> sortEarthquakes(String sortParameter) {
		...
		Comparator<EarthquakeMarker> comparatorType = (Comparator<EarthquakeMarker>) this
				.sortEarthquakes(sortParameter);
